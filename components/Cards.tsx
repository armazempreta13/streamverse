...
export const MediaCard = React.memo(function MediaCard({ title, subtitle, imageUrl, progress, slug, href, className, theme = 'default' }: MediaCardProps) {
  return (
    <Link
      ...
      className={`group cursor-pointer shrink-0 block relative ${className || 'w-[280px] sm:w-[300px] lg:w-[320px]'}`}
    >
      <div className={`aspect-[16/10] bg-[#0A0A16] relative overflow-hidden will-change-transform transition-transform duration-500 hover:-translate-y-2 z-10 ring-1 ring-white/5 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] ${theme === 'anime'
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-500 ease-out ${theme === 'anime' && siteConfig.features.crunchyrollStyleLayout
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 transform translate-y-1 group-hover:translate-y-0 transition-transform duration-500">
          <div className="flex justify-between items-end mb-1.5">
            <h3 className={`font-display font-bold text-[17px] leading-tight text-white line-clamp-1 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] transition-colors ${theme === 'anime' ? 'group-hover:text-[#FF3366]' : 'group-hover:text-[#A661FF]'
              }`}>
              {title}
            </h3>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <p className={`text-[10px] text-white/50 font-bold tracking-[0.15em] uppercase transition-colors ${theme === 'anime' ? 'group-hover:text-[#FF3366]/80' : 'group-hover:text-[#A661FF]/80'
                }`}>{subtitle}</p>
        ...
        {/* Hover Border Glow */}
        <div className={`absolute inset-0 border-2 transition-opacity duration-700 pointer-events-none ${theme === 'anime' ? 'border-[#FF3366]/30 opacity-0 group-hover:opacity-100 rounded-[8px]' : 'border-[#8F44FF]/20 opacity-0 group-hover:opacity-100 rounded-[16px]'
          }`} />
      </div>
    </Link>
  );
...
export const TrendingCard = React.memo(function TrendingCard({ title, rank, imageUrl, slug, href, theme = 'default' }: TrendingCardProps) {
  return (
    <Link
      href={href || `/content/${slug}`}
      className="w-[160px] sm:w-[200px] shrink-0 group cursor-pointer block relative"
    >
      <div className={`aspect-[2/3] bg-[#0A0A16] ring-1 ring-white/5 relative overflow-hidden will-change-transform transition-transform duration-700 hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.8)] group-hover:-translate-y-3 z-10 ${theme === 'anime'
        ...
        {theme === 'anime' && siteConfig.features.crunchyrollStyleLayout ? (
          ...
          <div className="absolute top-3 right-3 flex flex-col items-center">
            <div className={`bg-black/40 backdrop-blur-xl p-2.5 border border-white/10 flex flex-col items-center min-w-[40px] shadow-2xl transition-transform duration-500 origin-top-right group-hover:scale-110 ${theme === 'anime'
              ...
              <span className={`text-[10px] font-black uppercase tracking-[0.2em] mb-0.5 ${theme === 'anime' ? 'text-[#FF3366]' : 'text-[#A661FF]'
                }`}>Top</span>
        ...
        {/* Interactive Border */}
        <div className={`absolute inset-0 border transition-opacity duration-700 pointer-events-none opacity-0 group-hover:opacity-100 ${theme === 'anime'
